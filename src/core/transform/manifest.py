...
def build_manifest(
    *,
    ...
    # Each overlay points to a PNG generated by master_plan_visualize.
    # The mapping from overlay.id to PNG file is by convention:
    # processed/master_plan/<overlay.id>.png
    master_plan_png_dir = processed_dir / "master_plan"
    for overlay in master_plan_overlays:
        png_path = master_plan_png_dir / f"{overlay.id}.png"
        if not png_path.exists():
            logger.warning(
                "Skipping %s: PNG not found at %s. "
                "Run scripts/build_master_plan_pngs.py first.",
                overlay.id,
                png_path,
            )
            continue
        layers.append(
            LayerEntry(
                ...
                bounds_wgs84=_raster_bounds_wgs84(overlay.path),
                default_visible=False,
                default_opacity=0.5,
                extras={
                    "annex": overlay.annex,
                    "vectorized": overlay.vectorized,
                },
            )
        )
